...
* **basic**
  ...
  - *charges.txt*
    ...
    To start a new project, this file should be deleted. The algorithm will generate a new 
    one depends on the new molecule chemical mechanism. 
    
    To restart a project, this file can be kept, which can save time on getting parameters
    from the molecule 
...
## Getting start
1. Remove basic/charges.txt. Empty folder *typeSystem* and *rctSystem*
    - if tried to rerun the previous project, you can skip this step. The algorithm
        will find the charge.txt and import it.  
2. Prepare molecules (Currently only support mol2 file, each mol2 file contains one
    kind of molecule)
