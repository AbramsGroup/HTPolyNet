...
class Runtime:
    ...
    def generate_molecules(self,force_parameterization=False,force_checkin=False):
        ...
        logger.info('{:*^67s}'.format(' Generating molecular templates '))
        self.molecules={}
        ...
        logger.info('{:*^67s}'.format(f' {len(self.cfg.molecules)} molecule{ess} explicit in {self.cfgfile} '))
        ...
        if len(new_molecules)>0:
            ess='' if len(new_molecules)==1 else 's'
            logger.info('{:*^67s}'.format(f' {len(new_molecules)} molecule{ess} implied by chaining '))
            ml=list(new_molecules.keys())
            for i in range(0,len(ml),4):
                logger.info(4*'*'+' '+', '.join([m for m in ml[i:min(i+4,len(ml))]]))
            self.cfg.reactions.extend(new_reactions)
        ...
        logger.info('{:*^67}'.format(f' Generated {len(self.molecules)} molecule template{ess} '))
    ...
    def initialize_topology(self,inpfnm='init'):
        ...
        logger.info(f'Generated {inpfnm}.top in {pfs.cwd()}.')
