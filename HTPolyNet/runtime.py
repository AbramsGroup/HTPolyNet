...
class Runtime:
    runtime_defaults={
        ...
        'CURE': CureController.curedict_defaults,
        'postcure': {
            'anneal': {
                'ncycles': 0,
                'initial_temperature': 300,
    ...
    def __init__(self,cfgfile='',restart=False):
        ...
        self.cc=CureController(self.cfg.parameters['CURE'])
    ...
    def do_densification(self,deffnm='densified'):
        ...
        self._do_equilibration(equilibration,deffnm='densified')
...
#                    shutil.copy(f'molecules/parameterized/{mname}.{ex}','../../')
    ...
    def _do_equilibration(self,eq_dict={},deffnm='equilibrate'):
        gromacs_dict=self.cfg.parameters.get('gromacs',{})
        ...
        for stg in eq_dict:
            ...
            mod_dict={}
            if ens in ['nvt','npt']:
                mod_dict['ref_t']=stg['temperature']
                mod_dict['gen-temp']=stg['temperature']
                mod_dict['gen-vel']='yes'
                if ens=='npt': mod_dict['ref_p']=stg['pressure']
    ...
    def _do_pap(self,pfx=''):
        ...
        pfs.go_to(f'systems/{pfx}')
        preequil=pap_dict.get('preequilibration',{})
        anneal=pap_dict.get('anneal',{})
        postequil=pap_dict.get('postequilibration',{})
        if preequil: self._do_equilibration(preequil,deffnm='preequilibration')
        if anneal: self._do_anneal(anneal,deffnm='annealed')
        if postequil: self._do_equilibration(postequil,deffnme='postequilibration')
    ...
    def _do_anneal(self,anneal_dict={},deffnm='anneal'):
        ...
        my_logger(f'Anneal for {nsteps} steps',logger.info)
        mod_dict={
