...
class HTPolyNet:
    ...
    def generate_molecules(self,force_parameterization=False,force_checkin=False):
        ...
        logger.debug(f'Will generate the following: {msg}')
        ...
        #     logger.debug(f'Cycles in {M.name}')
        #     M.label_cycle_atoms()
    ...
    def CURE(self):
        ...
        while not CP.state==CPstate.postcure:
            ...
            if CP.state==CPstate.fresh:
                logger.info(f'CURE iteration {CP.iter}/{maxiter} begins in {cwd}.')
                CP.set_state(CPstate.bondsearch) # no need to write a checkpoint
            if CP.state==CPstate.bondsearch:
                stepno=0
                stepnm='bondsearch'
                opfx=f'{stepno}-{stepnm}'
                logger.info(f'{opfx}: CURE iteration {CP.iter}/{maxiter}: BONDSEARCH')
                ...
                while CP.state==CPstate.bondsearch and CP.current_radidx<max_radidx:
                    ...
                    if nbdf.shape[0]>0:
                        ...
                        logger.debug(f'{opfx}: CURE iteration {CP.iter}: increasing cutoff radius to {CP.radius+radial_increment}')
                        CP.current_radidx+=1
                        CP.radius+=radial_increment
                if CP.state==CPstate.bondsearch: # loop exited on radius violation
                    logger.debug(f'{opfx}: CURE iteration {CP.iter} failed to find bonds.')
                    cure_finished=True
            ...
            if CP.state==CPstate.equilibrate:
                ...
                stepnm='equilibrate'
                opfx=f'{stepno}-{stepnm}'
                logger.info(f'{opfx}: CURE iteration {CP.iter}/{maxiter}: EQUILIBRATION')
                CP.read_checkpoint(self)
        ...
        if CP.state==CPstate.finished:
            ...
            stepnm='final'
            opfx=f'{stepno}-{stepnm}'
            logger.info(f'{opfx}: CURE FINISHED')
            CP.read_checkpoint(self)
