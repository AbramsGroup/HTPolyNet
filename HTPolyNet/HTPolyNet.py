...
class HTPolyNet:
    ...
    def __init__(self,cfgfile='',restart=False):
        logger.info(software.to_string())
        ...
        if self.cfg.parameters['restart']:
            logger.info(f'***** THIS IS A RESTART *****')
        ...
        if not pfs.checkout(filename):
            ...
            for p in searchpath:
                ...
                if os.path.exists(fullfilename):
                    ...
                    logger.info(f'Checkout {fullfilename} to {os.getcwd()}')
                    return True
    ...
    def generate_molecules(self,force_parameterization=False,force_checkin=False):
        logger.info('*'*10+' GENERATING MOLECULE TEMPLATES '+'*'*10)
        self.molecules={}
        ...
        logger.info('*'*10+f' {len(self.cfg.molecules)} MOLECULES EXPLICIT IN {self.cfgfile} '+'*'*10)
        ...
        if len(symmetry_relateds)>0:
            ...
            logger.info('*'*10+f' {len(new_molecules)} MOLECULES IMPLIED BY MONOMER SYMMETRIES '+'*'*10)
        ...
        if len(new_molecules)>0:
            logger.info('*'*10+f' {len(new_molecules)} MOLECULES IMPLIED BY CHAINING '+'*'*10)
            logger.info(", ".join([m for m in new_molecules]))
            self.cfg.reactions.extend(new_reactions)
            for mname,M in new_molecules.items():
                if mname not in self.molecules:
        ...
        logger.info('*'*10+f' GENERATED {len(self.molecules)} MOLECULE TEMPLATES '+'*'*10)
    ...
    def generate_molecule(self,M:Molecule,**kwargs):
        ...
        if force_parameterization or not M.previously_parameterized():
            ...
            if generatable:
                logger.info(f'Yes -- calling {mname}.generate().')
        ...
        if len(M.stereocenters)>0:
            ...
            for p in P:
                ...
                logger.debug(f'Stereocenter sequence {p} generates stereoisomer {MM.name}')
    ...
    def initialize_topology(self,inpfnm='init'):
        ...
        logger.info(f'Global topology has {self.TopoCoord.Topology.atomcount()} atoms.')
        self.TopoCoord.write_top(f'{inpfnm}.top')
        logger.info(f'Wrote {inpfnm}.top to {cwd}')
    ...
    def setup_liquid_simulation(self,inpfnm='init'):
        ...
        if 'initial_boxsize' in self.cfg.parameters:
            ...
            aspect_ratio=self.cfg.parameters.get('aspect_ratio',[1,1,1])
            assert aspect_ratio[0]==1,f'Error: parameter aspect_ratio must be a 3-element-list with first element 1'
            ar_yx=aspect_ratio[1]*aspect_ratio[2]
            L0_m=(V0_m3/ar_yx)**(1./3.)
            L0_nm=L0_m*1.e9
            boxsize=np.array([L0_nm,L0_nm,L0_nm])*np.array(aspect_ratio)
            logger.info(f'Initial density {self.cfg.parameters["initial_density"]} kg/m^3 and total mass {mass_kg:.3e} kg and aspect ratio {aspect_ratio} dictate initial box side lengths of {boxsize[0]:.3f} nm x {boxsize[1]:.3f} nm x {boxsize[2]:.3f} nm')
        ...
        if not os.path.exists(f'{inpfnm}.gro') or not os.path.exists(f'{inpfnm}.top'):
            ...
            logger.info(f'Found {inpfnm}.gro.')
    ...
    def do_liquid_simulation(self,inpfnm='init',deffnm='npt-1'):
        ...
        if os.path.exists(f'{deffnm}.gro') and self.cfg.parameters['restart']:
            logger.info(f'{deffnm}.gro exists in {os.getcwd()}; skipping initial NPT md.')
        else:
            logger.info(f'Conducting initial NPT MD simulation of liquid')
            ...
            logger.info(f'Generated configuration {deffnm}.gro\n')
    ...
    def CURE(self):
        ...
        try:
            ...
            logger.error(f'CURE_desired_conversion not found in configuration.')
            logger.error(f'You must specify a desired conversion explicitly in the configuration file.')
            logger.error(f'For now, this is being defaulted to 0.5')
        ...
        while not CP.state==CPstate.postcure:
            ...
            if CP.state==CPstate.bondsearch:
                ...
                logger.info(f'CURE iteration {CP.iter}: Bondsearch using radius {CP.radius} nm initiated.')
                ...
                logger.debug(f'{opfx}: Iteration limited to at most {bond_limit} new bonds')
                ...
                while CP.state==CPstate.bondsearch and CP.current_radidx<max_radidx:
                    ...
                    if nbdf.shape[0]>0:
                        ...
                        logger.info(f'CURE iteration {CP.iter}: Increasing cutoff radius to {CP.radius+radial_increment} nm')
                        CP.current_radidx+=1
                        CP.radius+=radial_increment
                if CP.state==CPstate.bondsearch: # loop exited on radius violation
                    logger.info(f'CURE iteration {CP.iter} failed to find bonds.')
                    cure_finished=True
            if CP.state==CPstate.drag:
                ...
                stepnm='drag'
                opfx=f'{stepno}-{stepnm}'
                logger.info(f'CURE iteration {CP.iter}: Prebond dragging initiated on {CP.bonds.shape[0]} new bonds.')
                CP.read_checkpoint(self)
            ...
            if CP.state==CPstate.update:
                ...
                stepnm='update'
                opfx=f'{stepno}-{stepnm}'
                logger.info(f'CURE iteration {CP.iter}: Topology update')
                CP.read_checkpoint(self)
            ...
            if CP.state==CPstate.relax:
                ...
                stepnm='relax'
                opfx=f'{stepno}-{stepnm}'
                logger.info(f'CURE iteration {CP.iter}: Relaxation of {CP.bonds.shape[0]} bonds')
                CP.read_checkpoint(self)
                CP.bonds['initial_distance']=np.array(self.TopoCoord.return_bond_lengths(CP.bonds))
                CP.bonds['current_lengths']=CP.bonds['initial_distance'].copy()
                maxL,minL,meanL=CP.bonds['current_lengths'].max(),CP.bonds['current_lengths'].min(),CP.bonds['current_lengths'].mean()
            ...
            if CP.state==CPstate.equilibrate:
                ...
                stepnm='equilibrate'
                opfx=f'{stepno}-{stepnm}'
                logger.info(f'CURE iteration {CP.iter}: Equilibration')
                CP.read_checkpoint(self)
            ...
            if CP.state==CPstate.post_equilibration:
                ...
                if max_nxlinkbonds>0:
                    ...
                    logger.info(f'Iter {CP.iter} current conversion: {curr_conversion} ({curr_nxlinkbonds}/{max_nxlinkbonds})')
    ...
    def post_CURE(self,CP:Checkpoint):
    ...
    def register_system(self,CP:Checkpoint,extra_attributes=['z','nreactions','reactantName','cycle','cycle_idx','chain','chain_idx']):
        ...
        """
        logger.info(f'Writing system to {CP.top} {CP.gro} {CP.grx}')
...
def cli():
    ...
    logging.basicConfig(filename=args.log,filemode='w',format='%(asctime)s %(levelname)s %(name)s.%(funcName)s: %(message)s',level=loglevel_numeric)
