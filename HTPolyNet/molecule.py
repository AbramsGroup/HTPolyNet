...
class Molecule:
    ...
    def propagate_z(self,reactions,mdict):
        ...
        if self.generator:
            ...
            for resid,mname in R.reactants.items():
                logging.debug(f'taking z from {mname}')
                mol=mdict[mname]
                adf=mol.Coords.A
                zs=adf[adf['z']>0]
                for i,r in zs.iterrows():
                    ...
                    resName=r['resName']
                    z=r['z']
                    self.Coords.set_atom_attribute('z',z,{'atomName':atomName,'resName':resName,'resNum':resid})
            ''' correct for the bonds that form in this reaction '''
            for b in R.bonds:
                logging.debug(f'correcting for atoms in bond {b}')
                for k in b['atoms']:
                    a=R.atoms[k]
                    logging.debug(f'correction for atom {a}')
                    atomName=a['atom']
                    resNum=a['reactant']
                    resName=R.reactants[a['reactant']]
                    z=a['z']-1
                    self.Coords.set_atom_attribute('z',z,{'atomName':atomName,'resName':resName,'resNum':resNum})

            logging.debug(f'after propagate_z on as a product {self.name}:\n{self.Coords.A.to_string()}')

        ''' find any reaction in which this is a reactant '''
        atoms=[]
        for R in reactions:
            ''' is this reaction a proper generator or just a symmetry-clone? '''
            is_proper=False
            for mname,M in mdict.items():
                if M.generator and M.generator==R:
                    is_proper=True
                    break
            if self.generator and not is_proper:
                continue
            logging.debug(f'searching for {self.name} in {R.reactants.values()}')
            if self.name in R.reactants.values():  # this molecule is named as a reactant in this reaction
                ...
                logging.debug(f'{self.name} is reactant {ridx} in {R.name}')
        ...
        #     #             if setz>0 and z!=setz:
            #                 logging.warning(f'Error: we have a bad z-setting request on an atom whose z is already set')
            #             if setz==0:
            #                 logging.debug(f'Setting z of {self.name}-R{resNum}-{atomName}({Aidx}) to {z} ')
            #                 self.set_atom_attribute('z',z,{'globalIdx':Aidx})
            #                 # logging.info(f'Are we setting z values of symmetry-equivalent atoms?')
            #                 # if self.Coords.has_atom_attributes(['sea-idx']):
            #                 #     Asea=self.Coords.get_atom_attribute('sea-idx',{'globalIdx':Aidx})
            #                 #     Aclu=self.Coords.get_atoms_w_attribute('globalIdx',{'sea-idx':Asea})
            #                 #     logging.info(f'Yes.  Atom {self.name}-R{resNum}-{atomName}({Aidx}) belongs to sea-class {Asea} whose total member idx list is {Aclu}')
            #                 #     for aa in Aclu:
            #                 #         if aa!=Aidx:
            #                 #             an=self.Coords.get_atom_attribute('atomName',{'globalIdx':aa})
            #                 #             rn=self.Coords.get_atom_attribute('resNum',{'globalIdx':aa})
            #                 #             logging.info(f'Setting z of {self.name}-R{rn}-{an}({aa}) to {a["z"]}')
            #                 #             self.set_atom_attribute('z',z,{'globalIdx':aa})
            # elif self.name == R.product:
            #     logging.debug(f'{self.name} is the product of {R.name}')
            #     # inherit from the reactants
            #     for a in R.atoms.values():
            #         z=a['z']  # this is the z value listed in the cfg file
            #         atomName=a['atom']
            #         resNum=a['resid']
            #         resName=R.reactants[a['reactant']]
            #         Aclu=self.Coords.get_atoms_w_attribute('globalIdx',{'resName':resName,'atomName':atomName})
            #         for i in Aclu:
            #             self.set_atom_attribute('z',z,{'globalIdx':i})
