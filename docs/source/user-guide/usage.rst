...
``STY.mol2`` is a mol2 file describing styrene in this example.  ``HTPolyNet`` will store its parameterization information for all molecules and oligomersin the ``parameterized`` subdirectory.

``htpolynet run``
^^^^^^^^^^^^^^^^^

This subcommand is used to generate a polymerized system ("build a project") given directives in the configuration file and any required input monomer structures.
...
* ``-proj`` names the project directory.  If provided with the value ``next`` (the default), ``HTPolyNet`` will create the next autonamed project directory.  These are always named as ``proj-``n, where n is replaced by an integer beginning with 0; if no project directory exists and an explicit one is not specified by ``-proj``, ``HTPolyNet`` creates the first one, ``proj-0``.  A newly created project directory will automatically be given the following structure:
...
.. code-block:: console
  ...
  $ htpolynet parameterize [-h] [-lib LIB] [-diag DIAG] [-restart] [--force-parameterization] [--force-checkin] [--loglevel LOGLEVEL] config
